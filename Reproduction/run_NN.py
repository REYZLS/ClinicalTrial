...
# %%
train_data = pd.read_csv(f'time_prediction_train.csv', sep='\t')
test_data = pd.read_csv(f'time_prediction_test.csv', sep='\t')
...
incl_emb['train'], incl_mask['train'], excl_emb['train'], excl_mask['train'] = criteria2embedding(
    train_data['criteria'], padding_size)
incl_emb['test'], incl_mask['test'], excl_emb['test'], excl_mask['test'] = criteria2embedding(test_data['criteria'],
                                                                                              padding_size)
...
def drug2embedding(drug_lst):
    ...
    for drugs in tqdm(drug_lst):
        if len(drugs) == 0:
            ...
            # mean pooling
            drugs_emb = torch.mean(
                torch.stack([get_sentence_embedding(drug, tokenizer, model) for drug in drugs.split(';')]), dim=0)
...
def disease2embedding(disease_lst):
    ...
    for diseases in tqdm(disease_lst):
        if len(diseases) == 0:
            ...
            # mean pooling
            diseases_emb = torch.mean(
                torch.stack([get_sentence_embedding(disease, tokenizer, model) for disease in diseases.split(';')]),
                dim=0)
...
# %%
if not os.path.exists('drug_emb.pt') or not os.path.exists('disease_emb.pt'):
    drug_emb = {}
    ...
    torch.save(drug_emb, 'drug_emb.pt')
    torch.save(disease_emb, 'disease_emb.pt')
else:
    drug_emb = torch.load('drug_emb.pt')
    disease_emb = torch.load('disease_emb.pt')
...
class Trial_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.nctid_lst.iloc[idx], (self.incl_emb[idx], self.incl_mask[idx]), (self.excl_emb[idx],
                                                                                     self.excl_mask[idx]), \
        self.drug_emb[idx], self.dis_emb[idx], self.phase_emb[idx], self.target_lst[idx]


# %%
batch_size = 256

train_dataset = Trial_Dataset(train_data['nctid'], incl_emb['train'], incl_mask['train'], excl_emb['train'],
                              excl_mask['train'], drug_emb['train'], disease_emb['train'], phase_emb['train'],
                              train_data['time_day'])
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

test_dataset = Trial_Dataset(test_data['nctid'], incl_emb['test'], incl_mask['test'], excl_emb['test'],
                             excl_mask['test'], drug_emb['test'], disease_emb['test'], phase_emb['test'],
                             test_data['time_day'])
...
if use_valid:
    ...
    valid_dataset = Trial_Dataset(valid_data['nctid'], incl_emb['valid'], incl_mask['valid'], excl_emb['valid'],
                                  excl_mask['valid'], drug_emb['valid'], disease_emb['valid'], phase_emb['valid'],
                                  valid_data['time_day'])
...
def test(model, data_loader):
    ...
    for batch_idx, batch_data in enumerate(data_loader):
        nctid, (inclusion_emb, inclusion_mask), (exclusion_emb,
                                                 exclusion_mask), drug_emb, disease_emb, phase_emb, target = batch_data
        inclusion_emb, inclusion_mask, exclusion_emb, exclusion_mask, drug_emb, disease_emb, phase_emb, target = inclusion_emb.to(
            device), inclusion_mask.to(device), exclusion_emb.to(device), exclusion_mask.to(device), drug_emb.to(
            device), disease_emb.to(device), phase_emb.to(device), target.to(device)

        output = model.forward(inclusion_emb, inclusion_mask, exclusion_emb, exclusion_mask, drug_emb, disease_emb,
                               phase_emb)
        prediction = output[:, 0]
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            for batch_idx, batch_data in enumerate(train_loader):
                nctid, (inclusion_emb, inclusion_mask), (exclusion_emb,
                                                         exclusion_mask), drug_emb, disease_emb, phase_emb, target = batch_data
                inclusion_emb, inclusion_mask, exclusion_emb, exclusion_mask, drug_emb, disease_emb, phase_emb, target = inclusion_emb.to(
                    device), inclusion_mask.to(device), exclusion_emb.to(device), exclusion_mask.to(
                    device), drug_emb.to(device), disease_emb.to(device), phase_emb.to(device), target.to(device)

                output = protocol_model.forward(inclusion_emb, inclusion_mask, exclusion_emb, exclusion_mask, drug_emb,
                                                disease_emb, phase_emb)
                prediction = output[:, 0]
